...
class User(Thread):
    ...
    def run(self):
        while True:
            ...
            print(f'`{self.username}` отправил сообщение для `{receiver}`: `{message_text}`')
            time.sleep(1)
...
if __name__ == '__main__':
    ...
    fake = Faker()
    users_count = int(input('Введите количество пользователей для эмуляции = '))
    ...
    for x in range(users_count):
        ...
        print('Фейковые пользователи: ' + users[x])
        threads.append(User(
